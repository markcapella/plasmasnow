...
static int check_window_match(
    const xdo_t *xdo, Window wid, const xdo_search_t *search);
static int _xdo_match_window_class(
    const xdo_t *xdo, Window window, regex_t *re);
static int _xdo_match_window_classname(
    const xdo_t *xdo, Window window, regex_t *re);
static int _xdo_match_window_name(const xdo_t *xdo, Window window, regex_t *re);
static int _xdo_match_window_title(
    const xdo_t *xdo, Window window, regex_t *re);
...
static int _xdo_match_window_name(
    const xdo_t *xdo, Window window, regex_t *re) {
...
static int _xdo_match_window_pid(
    ...
    for (i = 0; i < nchildren; i++) {
        ...
        if (*windowlist_size == *nwindows_ret) {
            ...
            *windowlist_ret = (Window *)realloc(*windowlist_ret,
                *windowlist_size * sizeof(Window)); // wv
        }
    } /* for (i in children) ... */
